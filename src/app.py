...
def _sample_universe_from_csv(asof_date: str, size: int, seed: str | None) -> List[str]:
    return sample_universe(asof_date, size, seed)


def _sample_case(profile: str) -> Dict[str, Any]:
    asof_env = os.getenv("ASOF_DATE", "").strip()
    if asof_env:
        asof_date = asof_env
    else:
        asof_date = (datetime.now(timezone.utc).date() - timedelta(days=1)).isoformat()
    size = int(os.getenv("SAMPLE_UNIVERSE_SIZE", "5"))
    seed = os.getenv("RANDOM_SEED")
    universe = _sample_universe_from_csv(asof_date, size, seed)
    if not universe:
    ...
    aum_raw = os.getenv("PORTFOLIO_AUM") or os.getenv("AUM")
    if aum_raw:
        try:
            context["aum"] = float(aum_raw)
        except ValueError:
            pass
...
def _rule_status(rule_key: str, threshold, current, blocked: bool) -> str:
    if current in ("", None):
        return ""
    if rule_key == "blocklist":
        return "未达标" if blocked else "达标"
    try:
        cur = float(current)
        thr = float(threshold)
...
def _print_tables(result: Dict[str, Any], minimal_view: Dict[str, Any]) -> None:
    ...
    summary_rows = [
        ...
        ("binding_constraints", len(minimal_view.get("binding_constraints") or [])),
        ("recommended_actions", len(minimal_view.get("recommended_actions") or [])),
        ("recommendation", rec_summary),
        ("llm_used", minimal_view.get("llm_used")),
        ("llm_model", minimal_view.get("llm_model") or ""),
        ("supervisor_used", minimal_view.get("supervisor_used")),
    ...
    node_outputs = audit.get("node_outputs") or []
    ...
    audit_rows = [
        ...
        ("llm_model", audit.get("llm_model") or ""),
        ("gatekeeper_used", audit.get("gatekeeper_used")),
        ("gatekeeper_rationale", audit.get("gatekeeper_rationale") or ""),
        ("supervisor_used", audit.get("supervisor_used")),
        ("supervisor_rationale", audit.get("supervisor_rationale") or ""),
        ("candidate_nodes", ",".join(audit.get("candidate_nodes") or [])),
    ...
    if rules_snapshot:
        ...
        for key in sorted(rules_snapshot.keys()):
            ...
            status = _rule_status(key, val_fmt, cur_fmt, blocked)
            rules_rows.append((key, val_fmt, cur_fmt, status))
        print(_format_table(["RULE_KEY", "VALUE", "CURRENT", "STATUS"], rules_rows))
        print()

    if skills_used:
        skill_rows = [
            (s.get("name") or "", s.get("policy_version") or "", s.get("skills_hash") or "")
            for s in skills_used
        ]
        print(_format_table(["SKILL", "POLICY_VERSION", "HASH"], skill_rows))
        print()

    if node_outputs:
        node_rows = [(n,) for n in node_outputs]
        print(_format_table(["NODE_OUTPUT"], node_rows))
        print()
...
def main() -> None:
    ...
    case = _sample_case(args.profile)
