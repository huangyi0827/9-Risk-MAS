...
![Risk-MAS 系统架构图](Risk-mas框架图.png "Risk-MAS 多智能体风控系统架构")
...
- Python 3.12+
- uv（推荐）或 pip
- 关键依赖：LangGraph >= 1.0、LangChain >= 1.2、CVXPY >= 1.7
...
| 问题 | 可能原因 | 解决方案 |
|:---|:---|:---|
| `ModuleNotFoundError` | 依赖未安装 | 运行 `uv sync` 或 `pip install -e .` |
| LLM 调用超时 | 网络问题或 API 限流 | 检查 `OPENAI_BASE_URL`，或设置 `ENABLE_SUPERVISOR=0` |
| 数据文件找不到 | `CSV_DATA_DIR` 配置错误 | 确认路径正确，默认为 `cufel_practice_data` |
| 宏观节点被跳过 | 缺少 `TUSHARE_TOKEN` | 配置 Token 或忽略（非必需） |
| 合规检索返回空 | RAG 配置问题 | 检查 `COMPLIANCE_RAG_SOURCE`，尝试 `RAG_ENGINE=keyword` |
| 约束求解失败 | 约束冲突或数据异常 | 检查 `rules.yaml` 阈值是否合理 |
